...
runuser -u "$APP_USER" -- bash -lc "cd '$APP_DIR/frontend' && npm install --legacy-peer-deps && REACT_APP_BACKEND_URL='$FRONTEND_BACKEND_URL' REACT_APP_GOOGLE_CLIENT_ID='$GOOGLE_CLIENT_ID' REACT_APP_RECAPTCHA_SITE_KEY='$RECAPTCHA_SITE_KEY' npm run build"
rm -rf "$WEB_ROOT"
mkdir -p "$WEB_ROOT"
