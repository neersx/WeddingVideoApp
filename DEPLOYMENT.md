...
| backend (FastAPI) | 8001 | Only via Nginx `/api/*` | `STORAGE_BACKEND`, `MONGO_URL`, `DB_NAME`, `CORS_ORIGINS`, `RENDER_SERVICE_URL`, `INTERNAL_BASE_URL`, `GOOGLE_CLIENT_ID`, `ADMIN_EMAILS`, `RECAPTCHA_SECRET_KEY`, `RECAPTCHA_SCORE_THRESHOLD`, `RECAPTCHA_EXPECTED_ACTION` |
| frontend (static) | — | Yes (Nginx serves `/var/www/invitawedds/web/build`) | `REACT_APP_BACKEND_URL`, `REACT_APP_GOOGLE_CLIENT_ID`, `REACT_APP_ADMIN_EMAILS`, `REACT_APP_RECAPTCHA_SITE_KEY` (build-time only) |
