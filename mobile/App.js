...
export default function App() {
  ...
  const [request, response, promptAsync] = Google.useAuthRequest({ webClientId: GOOGLE_CLIENT_ID, iosClientId: process.env.EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID, androidClientId: process.env.EXPO_PUBLIC_GOOGLE_ANDROID_CLIENT_ID });
  ...
  async function choosePhotos() {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], allowsMultipleSelection: true, selectionLimit: 8, quality: 0.9 });
    if (!result.canceled) setPhotos(result.assets);
  }

  async function createVideo() {
    setError(''); setVideoUrl(null);
    if (!credential) return setError('Sign in with Google before creating a video.');
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="light" /><ScrollView contentContainerStyle={styles.container}>
    <Text style={styles.brand}>InvitaVideos</Text><Text style={styles.title}>Create your invitation reel</Text><Text style={styles.subtitle}>A polished video, ready to share.</Text>
