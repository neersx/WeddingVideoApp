...
import {TimelineData, TimelineMoment, TimelineScreen, WeddingProps} from './types';

// Shared building blocks for Timeline-category templates (Journey, Cascade …).
// Everything here is transition-agnostic: each template wraps these screens in
// its own entrance/exit choreography and lays out the Sequences itself.
...
export const OpeningScreen: React.FC<{screen: TimelineScreen; eyebrow: string; duration: number}> = ({
  screen,
  eyebrow,
  duration,
}) => (
...
export const MomentScreen: React.FC<{moment: TimelineMoment; index: number; total: number; duration: number}> = ({
  moment,
  index,
  total,
  duration,
...
export const ClosingScreen: React.FC<{screen: TimelineScreen; duration: number}> = ({screen, duration}) => (
  <AbsoluteFill>
...
// Resolves the timeline payload and the shared per-screen timing plan. Screens =
// opening + each moment + closing + branding; content screens split the
// composition evenly and branding takes the remainder.
export const useTimelinePlan = (props: WeddingProps) => {
  ...
  return {timeline, items, occasion, per, contentScreens, brandingStart, brandingDuration, musicVolume};
};
